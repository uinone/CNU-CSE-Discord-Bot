...
이후 `컴공과-공지사항`이라는 이름의 채팅채널을 하나 만들어주세요.
...
2. 해당 봇을 추가한 서버들의 특정 채팅채널(`이름은 "컴공과-공지사항"이어야해요!`)에서 볼 수 있도록 업데이트 된 게시글의 정보를 웹에서 긁어옵니다.
...
### 22-03-09 업데이트 확인 방식 변경

기존에 봇이 새로운 게시물이 올라왔음을 알게되는 방식은 게시물 번호였다.

문제는 조교님들이 특정 게시물을 공지로 띄우면 게시물 번호가 올라갔다.

아마 공지라고 올라가는 게시물은 내용은 똑같지만, 내부적으로는 새로운 게시물을 생성하는 것과 같아서 게시물 번호가 올라가는 것 같았다.

그러면 새로운 방식을 찾아야하는데, url에 있는 쿼리스트링을 보니 articleNo 라는 것이 있었다.

이를 새로운 확인 기준으로 삼아서 공지를 확인하기로 했다.
